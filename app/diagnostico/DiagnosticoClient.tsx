  ...
  return (
    <div style={{ width:'100vw', height:'100vh', background:'#010912', fontFamily:'Courier New, monospace', cursor:'none', overflow:'hidden', display:'flex', flexDirection:'column', position:'relative' }}>
      ...
      <div style={{ height:36, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', borderBottom:'0.5px solid rgba(0,180,216,0.15)', background:'rgba(1,9,18,0.9)', flexShrink:0, zIndex:10, position:'relative' }}>
        <span style={{ fontSize:9, letterSpacing:4, color:'#00B4D8', textShadow:'0 0 8px rgba(0,180,216,0.4)' }}>IMPULSO.AI // IMPETUS DIAGNOSTIC SYSTEM</span>
        ...
        <div style={{ flex:1, display:'flex', gap:8, padding:6, zIndex:10, position:'relative', perspective:'900px', perspectiveOrigin:'50% 45%', overflow:'hidden' }}>
          ...
          <div style={{ width:240, flexShrink:0, display:'flex', flexDirection:'column', gap:7, transform:'translateZ(18px) rotateY(-1.5deg)', transformStyle:'preserve-3d' }}>
            ...
            <div style={{ background:'rgba(0,25,52,0.85)', border:'0.5px solid rgba(0,180,216,0.3)', padding:'10px 8px 8px', flexShrink:0, position:'relative' }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#FFB400', textShadow:'0 0 10px rgba(255,180,0,0.5)', marginBottom:6 }}>NÚCLEO IMPETUS</div>
              ...
              <div style={{ textAlign:'center', marginTop:10 }}>
                ...
                <div style={{ fontSize:7, letterSpacing:3, color:'rgba(255,160,0,0.5)', marginTop:3, textShadow:'0 0 6px rgba(255,150,0,0.3)' }}>ANALISTA SR. IMPULSO.AI // GEN.4</div>
              </div>
            </div>

            <div style={{ background:'rgba(0,25,52,0.85)', border:'0.5px solid rgba(0,180,216,0.3)', padding:10, flexShrink:0, position:'relative' }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#00B4D8', opacity:0.75, marginBottom:6, textShadow:'0 0 8px rgba(0,180,216,0.5)' }}>LOCALIZAÇÃO</div>
            ...
            <div style={{ background:'rgba(0,25,52,0.85)', border:'0.5px solid rgba(0,180,216,0.3)', padding:10, flex:1, position:'relative' }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#00B4D8', opacity:0.75, marginBottom:6, textShadow:'0 0 8px rgba(0,180,216,0.5)' }}>DIAGNÓSTICO</div>
              {[
                ...
                <div key={b.label} style={{ marginBottom:5 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:2 }}>
                    <span style={{ fontSize:8, letterSpacing:2, color:'rgba(0,180,216,0.45)', textShadow:'0 0 4px rgba(0,180,216,0.2)' }}>{b.label}</span>
                    <span style={{ fontSize:9, color:b.color, textShadow:`0 0 6px ${b.color}55` }}>{b.val}</span>
                  </div>
              ...
              {[
                ...
                <div key={r.k} style={{ display:'flex', justifyContent:'space-between', padding:'3px 0', borderBottom:'0.5px solid rgba(0,180,216,0.07)' }}>
                  ...
                  <span style={{ fontSize:9, color:(r as any).c||'#00ffcc', textShadow:'0 0 5px rgba(0,255,200,0.3)' }}>{r.v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ flex:1, maxWidth:'55%', display:'flex', flexDirection:'column', gap:7 }}>
            <div style={{ background:'rgba(0,20,42,0.75)', border:'0.5px solid rgba(0,180,216,0.2)', padding:10, flex:1, display:'flex', flexDirection:'column', position:'relative' }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#00B4D8', opacity:0.75, marginBottom:6, textShadow:'0 0 8px rgba(0,180,216,0.5)' }}>INTERFACE DE DIAGNÓSTICO — SESSION #0041</div>
              ...
              <div ref={msgsRef} style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', gap:14, padding:4 }}>
                {messages.map((msg, i) => (
                  <div key={i} style={{ paddingTop:13, position:'relative', alignSelf: msg.role==='user' ? 'flex-end' : 'flex-start', maxWidth:'90%' }}>
                    <span style={{ position:'absolute', top:0, fontSize:7, letterSpacing:2, opacity:0.6, color: msg.role==='assistant' ? '#FFB400' : '#00ff88', textShadow: msg.role==='assistant' ? '0 0 6px rgba(255,180,0,0.5)' : '0 0 6px rgba(0,255,136,0.4)', [msg.role==='user'?'right':'left']:0 }}>
                      {msg.role==='assistant' ? 'IMPETUS' : 'OPERADOR'}
                    </span>
                    <div style={{ padding:'7px 10px', fontSize:10, lineHeight:1.6, background: msg.role==='assistant' ? 'rgba(0,40,70,0.8)' : 'rgba(0,50,25,0.7)', borderLeft: msg.role==='assistant' ? '1.5px solid rgba(0,180,216,0.6)' : 'none', borderRight: msg.role==='user' ? '1.5px solid rgba(0,255,136,0.6)' : 'none', color: msg.role==='assistant' ? '#b0e8ff' : '#b0ffcc', textAlign: msg.role==='user' ? 'right' : 'left' }}>
                      {msg.content}
                    </div>
                  </div>
                ))}
                {loading && (
                  <div style={{ paddingTop:13, position:'relative', alignSelf:'flex-start' }}>
                    <span style={{ position:'absolute', top:0, left:0, fontSize:7, letterSpacing:2, color:'#FFB400', opacity:0.6 }}>IMPETUS</span>
                    <div style={{ padding:'7px 10px', fontSize:10, color:'rgba(0,180,216,0.5)', background:'rgba(0,40,70,0.5)', borderLeft:'1.5px solid rgba(0,180,216,0.3)' }}>Processando...</div>
                  </div>
                )}
              </div>
              ...
              <div style={{ height:34, display:'flex', alignItems:'center', gap:8, background:'rgba(0,18,36,0.9)', border:'0.5px solid rgba(0,180,216,0.22)', padding:'0 10px', flexShrink:0 }}>
                ...
                <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSend()}
                  ...
                  style={{ flex:1, background:'transparent', border:'none', color:'#00ffcc', fontSize:10, letterSpacing:1, outline:'none', fontFamily:'Courier New', cursor:'none' }} />
              </div>
            </div>
          </div>
          ...
          <div style={{ width:200, flexShrink:0, display:'flex', flexDirection:'column', gap:7, transform:'translateZ(-16px) rotateY(1.5deg)', transformStyle:'preserve-3d' }}>
            ...
            <div style={{ background:'rgba(0,14,30,0.65)', border:'0.5px solid rgba(0,180,216,0.12)', padding:10, flexShrink:0, opacity:0.92, position:'relative' }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#00B4D8', opacity:0.75, marginBottom:6 }}>TIMESTAMP</div>
              <div style={{ fontSize:22, letterSpacing:3, color:'#00B4D8', textAlign:'center', fontWeight:'bold', textShadow:'0 0 18px rgba(0,180,216,0.5)' }}>{clock}</div>
            ...
            <div style={{ background:'rgba(0,14,30,0.65)', border:'0.5px solid rgba(0,180,216,0.12)', padding:10, flexShrink:0, opacity:0.92 }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#00B4D8', opacity:0.75, marginBottom:6 }}>EMPRESA ALVO</div>
              {[
                ...
                <div key={r.k} style={{ display:'flex', justifyContent:'space-between', padding:'3px 0', borderBottom:'0.5px solid rgba(0,180,216,0.07)' }}>
                  <span style={{ fontSize:8, letterSpacing:2, color:'rgba(0,180,216,0.4)' }}>{r.k}</span>
                  <span style={{ fontSize:9, color:(r as any).c||'#00ffcc' }}>{r.v}</span>
                </div>
              ))}
            </div>

            <div style={{ background:'rgba(0,14,30,0.65)', border:'0.5px solid rgba(0,180,216,0.12)', padding:10, flex:1, opacity:0.92 }}>
              <div style={{ fontSize:8, letterSpacing:3, color:'#00B4D8', opacity:0.75, marginBottom:6 }}>
              ...
              {problems.length === 0 ? (
                ...
                <div key={i} style={{ padding:6, border:'0.5px solid rgba(0,180,216,0.12)', marginBottom:4, position:'relative', background:'rgba(0,12,26,0.8)' }}>
                  <span style={{ position:'absolute', top:4, right:4, fontSize:7, letterSpacing:1, padding:'1px 4px', border:'0.5px solid', color: p.impacto==='alto'?'#ff5555':'#ffaa00', borderColor: p.impacto==='alto'?'rgba(255,85,85,0.4)':'rgba(255,170,0,0.4)', background: p.impacto==='alto'?'rgba(255,85,85,0.1)':'rgba(255,170,0,0.08)' }}>
                    {p.impacto.toUpperCase()}
                  </span>
                  <div style={{ fontSize:8, letterSpacing:2, color:'#00B4D8', marginBottom:2, textShadow:'0 0 6px rgba(0,180,216,0.4)' }}>{p.area.toUpperCase()}</div>
      ...
      <div style={{ height:36, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px', borderTop:'0.5px solid rgba(0,180,216,0.15)', background:'rgba(1,9,18,0.9)', flexShrink:0, zIndex:10, position:'relative' }}>
        <div style={{ display:'flex', alignItems:'center', gap:5 }}>
          ...
          <span style={{ fontSize:9, color:'#00B4D8', textShadow:'0 0 6px rgba(0,180,216,0.4)' }}>v4.3</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:5 }}>
          <span style={{ fontSize:6, letterSpacing:2, color:'rgba(0,180,216,0.3)' }}>SESSÃO</span>
          <span style={{ fontSize:9, color:'#00B4D8' }}>#0041</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:5 }}>
          <span style={{ fontSize:6, letterSpacing:2, color:'rgba(0,180,216,0.3)' }}>TOKENS</span>
          <span style={{ fontSize:9, color:'#00B4D8' }}>{tokenCount.toLocaleString()}</span>
        </div>
        {phase === 'chat' && (
